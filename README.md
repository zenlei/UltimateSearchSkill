...
默认 `docker-compose.yml` 将服务仅绑定到本机回环（外网不可直接访问）：

- Grok2API：`127.0.0.1:8100 -> 8000`
- TavilyProxyManager：`127.0.0.1:8200 -> 8080`
- FlareSolverr：仅 Docker 内网可访问（grok2api 使用 `http://ultimate-search-flaresolverr:8191`）

如需远程管理，建议使用 SSH 隧道，而不是把端口直接暴露到公网。
...
**方式一：少量 Token（推荐）— 用 import-keys.sh**

```bash
# 确保 export_sso.txt 在项目根目录（每行一个 token）
bash scripts/import-keys.sh
```

**方式二：大量 Token（推荐）— 用 import-multi-keys.sh 分批导入**

当你有上千条 SSO token 时，单次 POST 很容易遇到请求体过大 / 超时 / ARG_MAX（参数列表过长）。
此时请用分批脚本（默认会“合并去重”，因为 grok2api 的 `/v1/admin/tokens` 行为是“覆盖式”而非追加式）：

```bash
bash scripts/import-multi-keys.sh --batch-size 50
# 更稳：
# bash scripts/import-multi-keys.sh --batch-size 10
```

> ⚠️ 不要用 `--no-merge`：我们已验证 `/v1/admin/tokens` 是覆盖式（POST 会覆盖池子）。
...
> ✅ 本仓库已在 grok2api 配置中启用 FlareSolverr（proxy.enabled=true + flaresolverr_url 指向容器名），
> 正常情况下无需手动改 `config.toml`。
> 若你使用旧版本仓库或 data 目录被复用，才可能需要下面的手工校正。
...
> ⚠️ `TAVILY_MASTER_KEY` 是一个较长的随机串（通常 40+ 字符）。如果你手动复制时被截断，会导致所有 API 401。
> 推荐让脚本从日志自动抓取并写回 `.env`，或直接从 `docker compose logs tavily-proxy` 中复制完整值。
...
#### 本地网络需要代理（SOCKS5/HTTP）时
如果你的网络环境要求容器通过代理访问 grok.com / ghcr.io / tavily 等：

- `docker-compose.yml` 已支持透传 `HTTP_PROXY/HTTPS_PROXY/ALL_PROXY/NO_PROXY` 到容器。
- Ubuntu/Linux 下已加入 `host.docker.internal:host-gateway`，因此代理地址可以写：
  `host.docker.internal:<PORT>`

示例（SOCKS5，端口 20170）：

```bash
# 写入 .env（docker compose 会自动读取，不用每次 export）
ALL_PROXY=socks5://host.docker.internal:20170
NO_PROXY=localhost,127.0.0.1,grok2api,tavily-proxy,flaresolverr
```

> ⚠️ 宿主机代理若只监听 127.0.0.1，容器将无法连接。
> 需要让代理监听到 0.0.0.0 或至少 docker0 网卡（否则会出现 `ERR_PROXY_CONNECTION_FAILED`）。
...
bash scripts/web-fetch.sh --url "https://docs.openclaw.ai/"
```
...
### 接入 OpenClaw（让 agent 自动使用搜索能力）

把 skill 放到 OpenClaw workspace 的 `skills/` 下后（例如 `~/.openclaw/workspace/skills/ultimate-search`），
确保该目录下存在 `.env`（推荐用软链接指向你项目根目录的 `.env`）：

```bash
ln -sf ~/UltimateSearchSkill/.env ~/.openclaw/workspace/skills/ultimate-search/.env

# 检查是否 Ready
openclaw skills info ultimate-search
```

> 沙箱提示：如果未来开启 OpenClaw sandbox（Docker 工具沙箱），sandbox **不会继承宿主机环境变量**。
> 这时需要把 GROK/TAVILY/FIRECRAWL 相关 env 注入 sandbox（见 OpenClaw 文档 Sandboxing）。
